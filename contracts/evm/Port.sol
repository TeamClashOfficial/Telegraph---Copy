...
contract DataLayout is LibraryLock {
    ...
    mapping(string => bool) public isMainnet;
}
...
contract PortContract is ERC20, Ownable, Proxiable, DataLayout {
    ...
    function updateCode(
        address newCode
    ) public delegatedOnly onlyApproved("updateCode") {
    ...
    // Requires no previous authorization--Just superadmin status
    function superAdminVoteNewSuperAdmin(address owner) public {
        AuthLib.voteNewOwner(auth, owner);
    }

    // Requires no previous authorization--Just superadmin status
    function superAdminRemoveSuperAdmin(address ownerToRemove) public {
    ...
    function setEntryFees(
        ...
        uint256[] memory _fee
    ) public onlyApproved("setEntryFees") {
        require(AuthLib.isOwner(auth), "AuthLib: caller is not the owner");
    ...
    function setChainType(
        ...
        bool _isMainnet
    ) public onlyApproved("setChainType") {
        require(AuthLib.isOwner(auth), "AuthLib: caller is not the owner");
        isMainnet[_chain] = _isMainnet;
        AuthLib.clearAuth(auth, "setChainType");
    }

    function setDistributionContract(
        address _contract
    ) public onlyApproved("setDistributionContract") {
        require(AuthLib.isOwner(auth), "AuthLib: caller is not the owner");
    ...
    function superAdminAddSigner(
        ...
        AuthLib.clearAuth(auth, "superAdminAddSigner");

        emit NewSigner(_signer, _domain);
    }

    function superAdminRemoveSigner(
        address _signer
    ) public onlyApproved("superAdminRemoveSigner") {
        require(AuthLib.isOwner(auth), "AuthLib: caller is not the owner");
        require(isValidSigner[_signer], "Signer not valid or already removed");

        isValidSigner[_signer] = false;

        for (uint i = 0; i < signersArr.length; i++) {
            if (signersArr[i] == _signer) {
        ...
        AuthLib.clearAuth(auth, "superAdminRemoveSigner");

        emit SignerRemoved(_signer);
    }

    function superAdminEmptySigners()
        public
        onlyApproved("superAdminEmptySigners")
    {
        require(AuthLib.isOwner(auth), "AuthLib: caller is not the owner");
        for (uint i = 0; i < signersArr.length; i++) {
            isValidSigner[signersArr[i]] = false;
        }

        delete signersArr;
        AuthLib.clearAuth(auth, "superAdminEmptySigners");
    }

    function getSuperAdmins() public view returns (address[] memory) {
        address[] memory admins = AuthLib.getOwners(auth);
        return admins;
    }
    ...
    function outboundMessage(
        ...
        bool startChainIsMainnet = isMainnet[startChain];
        bool endChainIsMainnet = isMainnet[endChain];
        require(
            startChainIsMainnet == endChainIsMainnet,
            "Cannot send messages between mainnet and testnet"
        );
    ...
    function inboundMessage(
        ...
        DestinationContract(destination).portMessage(sender, data, _startChain);
    ...
    function executeInboundMessage(
        ...
        bool currentChainIsMainnet = isMainnet[startChain];
        bool senderChainIsMainnet = isMainnet[_startChain];
        require(
            currentChainIsMainnet == senderChainIsMainnet,
            "Cannot accept messages between mainnet and testnet"
        );
        require(
            signatureCheck(sigV, sigR, sigS, hashes),
            "Signer threshold not met"
